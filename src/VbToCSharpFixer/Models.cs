...
public enum ReasonCode { UnresolvedSymbol, MissingReference, AmbiguousSymbol, UnsupportedDefaultProperty, UnsupportedIndexer, ProjectLoadFailure, CompilationError, UnsupportedSyntax }
...
public sealed record Options(
    ...
    string Output, bool DryRun, bool Verbose)
{
    public static Options Parse(string[] args)
    {
        ...
        return new(solution, project, folder, file, Path.GetFullPath(output), dryRun, verbose);
    }

    private static string Next(string[] args, ref int i) =>
        ++i < args.Length ? Path.GetFullPath(args[i]) : throw new ArgumentException(Usage);

    public const string Usage = "Usage: VbToCSharpFixer (--solution x.sln | --project x.vbproj | --folder dir | --file x.vb) --output dir [--dry-run] [--verbose]";
}
